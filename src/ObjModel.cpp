...
void loadFromObj(char* path, Model& model) {
	std::string strPath = std::string(path);
    Logger::logInfo("Loading OBJ file from " + strPath);

    std::ifstream objFile(strPath);
    if (!objFile.is_open()) {
        Logger::logError("Failed to parse file " + strPath);
        return;
    }
    ...
    if (objFile.bad()) {
        Logger::logError("Error while reading OBJ file from " + strPath);
        return;
    }
