...
void from_json(const nlohmann::json &j, ParticleColorConfig &colorConfig) {
	glm::from_json(j, "startColor", colorConfig.startColor);
	glm::from_json(j, "endColor", colorConfig.endColor);
	j.at("isColorStatic").get_to(colorConfig.isColorStatic);
	j.at("function").get_to(colorConfig.function);
}
...
void from_json(const nlohmann::json &j, ParticleEmitter &emitter) {
    ...
    j.at("colorConfig").get_to(emitter.colorConfig);
    j.at("movementFunction").get_to(emitter.movementFunction);

	emitter.generate();
}

glm::vec3 ParticleFunctionConfig::calculate(float t) const {
	return initialVelocity + (linear * t) + (quadratic * t * t);
}
...
void ParticleEmitter::generate() {
  ...
  for (int vidx = 0; vidx < numVertices; vidx++) {
	...
	if (colorConfig.isColorStatic) {
		vertices[index + 3] = colorConfig.startColor.x;
		vertices[index + 4] = colorConfig.startColor.y;
		vertices[index + 5] = colorConfig.startColor.z;
	}
  ...
  lifeFrequencyDistribution  = std::uniform_int<uint32_t>(particleLifeRangeSeconds[0], particleLifeRangeSeconds[1]);
  spawnFrequencyDistribution = std::uniform_int<uint32_t>(spawnFrequencyRangeSeconds[0], spawnFrequencyRangeSeconds[1]);
