...
glm::mat4 get_light_projection(const Light &light) {
    ...
    case Directional:
      return glm::ortho<float>(-10, 10, -10.f, 10, Constants.near,
                                 Constants.far);
    case Spot:
      return glm::perspective<float>(glm::radians<float>(45.f),
...
glm::mat4 get_light_view(const Light &light) {
    switch (light.type) {
    case Directional:
        return glm::lookAt((-Constants.far / 2.0f) * light.direction,
                           light.direction, light.up);
    case Spot:
        return glm::lookAt(light.position, light.direction, light.up);
    case Point:
    default:
        cerr << "Attempting to get a view for unknown light: " << light.type
             << endl;
...
void to_json(json &j, const Scene &scene) {
    j = json{{"models", scene.get_models()}, {"lights", scene.get_lights()}};
}
